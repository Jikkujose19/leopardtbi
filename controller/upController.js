    ...
    if (token) {
        jwt.verify(token, 'secret', function(err, decoded) {       
            if (err) {
                ...
                Up.find({ lid: decoded.loginId }, { title: 1, amount: 1, category: 1, filename: 1 }, function(err, u) {
                    if(err) {
                        //handle error here
                        res.send(err);
                     }
    ...
    if (token) {
        jwt.verify(token, 'secret', function(err, decoded) {       
            if (err) {
                ...
                Up.find({}, {  title: 1, amount: 1, category: 1, filename: 1 }, function(err, u) {
                    if(err) {
                        //handle error here
                        res.send(err);
                     }
    ...
    if (token) {
        jwt.verify(token, 'secret', function(err, decoded) {       
            if (err) {
                ...
                Up.find({ _id: req.body.I_id }, {  title: 1, amount: 1, category: 1, filename: 1, create_date: 1 }, function(err, u) {
    ...
    if (token) {
        jwt.verify(token, 'secret', function(err, decoded) {       
            if (err) {
                ...
                Up.deleteOne({ _id: req.body.I_id, lid: decoded.loginId }, function (err, r) {
